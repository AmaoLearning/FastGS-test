...
def spatial_regularization_loss(
    ...
    compute the KL divergence to each of its *k* nearest neighbors' probabilities,
    then average over all neighbors and all Gaussians:

    .. math::

        \\mathcal{L} = \\frac{1}{N \\cdot k} \\sum_i \\sum_{j \\in \\mathcal{N}(i)}
            \\mathrm{KL}\\bigl(\\mathrm{Bern}(p_i) \\|
            \\mathrm{Bern}(p_j)\\bigr)

    This encourages spatial consistency: each Gaussian's probability should
    match its neighbors' probabilities individually (stronger constraint than
    matching the mean).
