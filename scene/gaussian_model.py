...
class GaussianModel:
    ...
    def densify_and_prune_ablation(self, max_screen_size, min_opacity, extent, radii, args, importance_score = None, pruning_score = None, densify_fastgs = True, prune_fastgs = True):
        
        ''' 
            Densification and Pruning either following FastGS or 3DGS. Work as an ablation study.
        '''
        grad_vars = self.xyz_gradient_accum / self.denom
        grad_vars[grad_vars.isnan()] = 0.0
        self.tmp_radii = radii
        ...
        if densify_fastgs:
            ...
            self.densify_and_clone_3dgs(grad_vars, max_grad, extent)
            self.densify_and_split_3dgs(grad_vars, max_grad, extent)
    ...
    def densify_and_split_3dgs(self, grads, grad_threshold, scene_extent, N=2):
        n_init_points = self.get_xyz.shape[0]
